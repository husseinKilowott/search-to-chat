...
function updateButtonStyles() {
  ...
  if (bubbleIconUrl !== null) {
    button.innerHTML = `<img src="${bubbleIconUrl}" alt="chat-icon" style="width: 25px; height: 25px; object-fit: contain;" />`;
  } else {
...
function openIframe() {
  ...
  if (!iframeContainer) {
    // Lazy loading: Create iframe and container when button is clicked for the first time
    ...
    closeButton.classList.add("close-iframe-btn");
    closeButton.innerHTML = closeIcon;
    // Set iframe src dynamically based on the environment
    if (window.chatConfig.env == "skillbuilder" || window.chatConfig.env == "skl") {
      if(prevSessionId){
        iframe.src = `https://skillbuilder.io/external-ai-chat/${window.chatConfig.chatId}?prevSessionId=${prevSessionId}`;
      }else{
        iframe.src = `https://skillbuilder.io/external-ai-chat/${window.chatConfig.chatId}`;
      }
    } else {
      if(prevSessionId){
        iframe.src = `https://${window.chatConfig.env}.skillbuilder.io/external-ai-chat/${window.chatConfig.chatId}?prevSessionId=${prevSessionId}`;
      }else{
        iframe.src = `https://${window.chatConfig.env}.skillbuilder.io/external-ai-chat/${window.chatConfig.chatId}`;
      }
      
    }
    var closeIframeButton = document.createElement("button");
    ...
    // Append iframe container to the document body
  ...
  // Toggle iframe visibility and button state
  if (
    iframeContainer.style.display === "none" ||
    !iframeContainer.style.display
  ) {
    // Show iframe container without reloading iframe content
    button.style.display = "none";
    iframeContainer.style.display = "block";
    ...
    document.body.style.overflow = "hidden"; // Disable scrolling when iframe is visible
  }
}
